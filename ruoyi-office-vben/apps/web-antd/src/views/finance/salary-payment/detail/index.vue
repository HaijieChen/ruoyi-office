...
/**
 * 薪资付款 · BPM 自定义表单「查看」组件（F3 + EXP-87 F1/F6）。
...
function appliedAccountId(entityCompanyDeptId?: number) {
  const lines = detail.value?.salaryLines || [];
...
async function loadData() {
  ...
  try {
    ...
    detail.value = null;
    message.error(error instanceof Error ? error.message : '加载薪资付款详情失败');
  } finally {
    loading.value = false;
  }
}
