...
const processDefineKey = 'oa_punch_correction';
const activityNodes = ref<BpmProcessInstanceApi.ApprovalNodeInfo[]>([]);
const processDefinitionId = ref('');
const resubmitSourceId = ref<number>();

const getTitle = computed(() =>
  resubmitSourceId.value
    ? '重提补卡'
    : $t('ui.actionTitle.create', ['补卡']),
);
...
async function onSubmit() {
  ...
  try {
    ...
    await router.push({ name: 'OAPunchIndex' });
  } catch {
...
onMounted(async () => {
  ...
  if (!processDefinitionDetail) {
    message.error('OA 补卡的流程模型未配置，请检查！');
    return;
  }
    ...
    <Row :gutter="16">
      <Col :span="16">
        <Card :title="getTitle" class="w-full" v-loading="formLoading">
          ...
          <FormBody
            ref="formBodyRef"
            @predict-change="getApprovalDetail"
            @success="() => {}"
          />
          <template #actions>
