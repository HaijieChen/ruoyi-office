...
/**
 * 勾选岗位，只取发起任职公司下持有该岗位的用户。
 */
@Component
public class BpmTaskCandidateStartUserCompanyPostStrategy implements BpmTaskCandidateStrategy {
    ...
    public BpmTaskCandidateStrategyEnum getStrategy() {
        return BpmTaskCandidateStrategyEnum.START_USER_COMPANY_POST;
    }
    ...
    Set<Long> resolve(String param, Long startUserId, Map<String, Object> processVariables) {
        ...
        List<AdminUserRespDTO> users = adminUserApi
                .getUserListByPostIds(StrUtils.splitToLongSet(param))
                .getCheckedData();
        ...
        for (AdminUserRespDTO user : users) {
            ...
            if (companyId.equals(BpmStartCompanyDeptSupport.findCompanyId(user.getDeptId(), deptApi))) {
                ids.add(user.getId());
            }
        }
        return ids;
