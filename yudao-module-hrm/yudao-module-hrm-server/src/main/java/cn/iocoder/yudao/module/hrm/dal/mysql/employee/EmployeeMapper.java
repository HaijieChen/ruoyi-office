...
public interface EmployeeMapper extends BaseMapperX<EmployeeDO> {
    ...
    /**
     * 当前租户纯数字工号中数值最大的原样字符串。
     */
    default String selectMaxNumericEmployeeNo() {
        EmployeeDO maxEmployee = selectOne(new LambdaQueryWrapperX<EmployeeDO>()
                ...
                .last("ORDER BY CAST(employee_no AS UNSIGNED) DESC, CHAR_LENGTH(employee_no) DESC LIMIT 1"));
        return maxEmployee == null ? null : maxEmployee.getEmployeeNo();
    }
     ...
     */
    default List<EmployeeDO> selectListByJobPositions(Collection<String> jobPositions) {
        if (jobPositions == null || jobPositions.isEmpty()) {
            return List.of();
        }
        return selectList(new LambdaQueryWrapperX<EmployeeDO>()
                .in(EmployeeDO::getJobPosition, jobPositions)
