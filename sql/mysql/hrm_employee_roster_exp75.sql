...
-- 权威优先级：
--   1) 存在唯一 BINARY URL 候选 → 必须纠正为该 fileId（覆盖错误 file_id+path）
--   2) 否则 file_id+path 与 infra_file 一致可保留
--   3) 否则唯一 BINARY path 候选
-- 活动 + 软删行均做 URL/fileId/path 纠正与 path 规范化；仅步骤 12 入职单复制限活动源
--
-- 【发布前快照 / 回滚】
-- 1) before-image: mysqldump common_attachment(id,business_type,file_id,file_path,file_url,deleted,tenant_id)
--                  与 infra_file(id,path,url,deleted)
-- 2) 回滚: 按 id 恢复 file_id/file_path/file_url
-- 3) 完整脚本必须连跑两次（H1 错误 fileId vs 唯一 URL、H2 软删 URL-only）

-- 11a. 清空 missing/soft-deleted infra 目标（活动+软删附件）
...
-- 11b. 有唯一 BINARY URL 时强制纠正 file_id（覆盖错误 file_id+path）——含软删
UPDATE `common_attachment` a
INNER JOIN (
...
WHERE LOWER(TRIM(a.business_type)) IN ('hrm_employee_archive_onboarding', '201')
  ...
  AND a.file_url <> ''
  AND (a.file_id IS NULL OR a.file_id <> cand.fid);

-- 11c. 清空仍非权威的 file_id（仅当无唯一 URL 时，file_id+path 才可作权威）
UPDATE `common_attachment` a
SET a.file_id = NULL
WHERE LOWER(TRIM(a.business_type)) IN ('hrm_employee_archive_onboarding', '201')
  AND a.file_id IS NOT NULL
  AND NOT (
    -- 唯一 URL 权威（file_id 已是该 URL 候选）
    EXISTS (
      SELECT 1 FROM (
        SELECT BINARY f.url AS u, MIN(f.id) AS fid
        FROM `infra_file` f
...
-- 11d. 唯一 BINARY path 回填（无 file_id 且无唯一 URL）——含软删
UPDATE `common_attachment` a
INNER JOIN (
  SELECT BINARY f.path AS p, MIN(f.id) AS fid
  FROM `infra_file` f
...
-- 11e. path 规范为权威 infra_file.path（活动+软删；URL 纠正后可续跑）
UPDATE `common_attachment` a
...
-- 12. 历史入职单附件 → 员工档案入职资料（仅活动源；含空白 ' 201 '）
...
-- 13. 转档后：对 onboarding（含软删）再跑 URL 优先纠正 + path 规范
UPDATE `common_attachment` a
INNER JOIN (
  SELECT BINARY f.url AS u, MIN(f.id) AS fid
  FROM `infra_file` f
