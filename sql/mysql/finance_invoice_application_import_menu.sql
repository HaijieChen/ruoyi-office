...
SELECT '开票申请导入', 'finance:invoice-application:import', 3, 20, parent.id, '', '', NULL, NULL,
       0, b'1', b'1', b'1', 'admin', NOW(), 'admin', NOW(), b'0'
FROM `system_menu` parent
WHERE parent.`deleted` = b'0' AND parent.`component` = 'finance/invoice-application/index'
  AND NOT EXISTS (
      SELECT 1 FROM `system_menu` m
      WHERE m.`deleted` = b'0' AND m.`permission` = 'finance:invoice-application:import'
  );
...
WHERE r.deleted=b'0' AND r.tenant_id=1 AND r.code IN ('finance_admin', 'business_staff')
  AND m.deleted=b'0' AND m.permission='finance:invoice-application:import'
  AND NOT EXISTS (
