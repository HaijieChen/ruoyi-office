-- OA 补卡菜单权限（幂等；挂在请假同一 BPM OA 父级下，非 finance）
...
SELECT '补卡查询', '', 2, 4,
       ...
       'punch', 'fa:calendar-check-o', 'bpm/oa/punch/index', 'BpmOAPunch',
...
WHERE (SELECT `parent_id` FROM `system_menu` WHERE `deleted` = b'0' AND `component` = 'bpm/oa/leave/index' LIMIT 1) IS NOT NULL
  AND NOT EXISTS (
    ...
    WHERE `deleted` = b'0' AND `component` = 'bpm/oa/punch/index'
);
...
SELECT btn.name, btn.permission, 3, btn.sort, parent.id, '', '', NULL, NULL,
       0, b'1', b'1', b'1', 'admin', NOW(), 'admin', NOW(), b'0'
FROM (
    SELECT '补卡申请查询' AS name, 'bpm:oa-punch-correction:query' AS permission, 1 AS sort
    UNION ALL SELECT '补卡申请创建', 'bpm:oa-punch-correction:create', 2
) btn
CROSS JOIN (
    SELECT `id` FROM `system_menu`
    WHERE `deleted` = b'0' AND `component` = 'bpm/oa/punch/index' LIMIT 1
) parent
WHERE NOT EXISTS (
...
WHERE r.`deleted` = b'0' AND r.`code` IN ('hr_admin', 'super_admin') AND r.`tenant_id` = 1
  ...
  AND (
     ...
     OR m.`component` = 'bpm/oa/punch/index'
     OR m.`permission` = 'bpm:oa-punch-correction:query'
  )
  AND NOT EXISTS (
