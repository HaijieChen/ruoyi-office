-- 合同签约申请台账 + 商务单正式关联列（CS-T1）
-- 幂等：CREATE IF NOT EXISTS + information_schema 判列后 PREPARE 加列；可重复执行
-- 参考：tech-plan-contract-signing §4 D-T6/D-T7；contract-signing-decisions C21/C22
...
-- 一期不做列：审批流程已完成证明（C22）——不建列

-- ---------------------------------------------------------------------------
-- 2. 商务单：正式关联合同申请（保留 contract_process_id 作 legacy）
-- ---------------------------------------------------------------------------
